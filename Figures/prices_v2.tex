...
\begin{subfigure}{6.2cm}
    ...
    \addplot[name path=upper, violet!40] table [x=Y, y=λ_ets_scenario_1, col sep=comma]{\datae};
    \addplot[name path=lower, violet!40] table [x=Y, y=λ_ets_scenario_1, col sep=comma]{\data};
    \addplot[fill=violet, fill opacity=0.4] fill between[of=upper and lower];
    % Middle line as the average scenario
    \addplot[line width=1.5pt, violet] table [x=Y, y=λ_ets_scenario_1, col sep=comma]{\datac};
    % Dotted line as reference price
    \addplot[line width=1pt, black,dashed] table [x=Y, y= λ_ets_scenario_1, col sep=comma]{\refr};
    \end{axis}
    \end{tikzpicture}
\end{subfigure}
...
\begin{subfigure}{8cm}
    ...
    \begin{axis}[
         ...
         /pgf/number format/1000 sep={},
          legend pos=outer north east]
    % Shaded area between the first and fifth lines
    \addplot[name path=upper, teal!40] table [x=Y, y=λ_product_scenario_1, col sep=comma]{\datae};
    \addplot[name path=lower, teal!40] table [x=Y, y=λ_product_scenario_1, col sep=comma]{\data};
    \addplot[fill=teal, fill opacity=0.4] fill between[of=upper and lower];
    % Middle line as the average scenario
    \addplot[line width=1.5pt, teal] table [x=Y, y=λ_product_scenario_1, col sep=comma]{\datac};
    % Dotted line as reference price
    \addplot[line width=1pt, black, dashed] table [x=Y, y= λ_product_scenario_1, col sep=comma]{\refr};
    \end{axis}
    \end{tikzpicture}
\end{subfigure}
\end{figure}
